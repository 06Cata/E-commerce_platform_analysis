...
def crawl_title(query,i,min_comment_length):
    data1 = [] # 全部文章的資料
    ptts=['e-shopping','WomenTalk','E-appliance','MobileComm','PC_Shopping','nb-shopping','Audiophile','watch','Digitalhome','MakeUp','BeautySalon','Lifeismoney','Gossiping']
    num = 0
    ...
    for ptt in ptts:
        ...
        for page in range(1,i):
            ...
            # 如果文章已被刪除，連結為None
                ...
                # 找到上頁的網址，並更新url
                # base_url = "https://www.ptt.cc/"+res.find("a", string="‹ 上頁")["href"]
...
# def get_ptt_posts(url, min_comment_length):
#     response = requests.get(url)
#     soup = BeautifulSoup(response.text, 'html.parser')

#     posts = []
#     comments = soup.select('.push .push-content')
#     for comment in comments:
